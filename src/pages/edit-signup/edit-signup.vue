<template>
	...
	<view class="container" v-else>
		...
		<up-form :model="formData" :rules="rules" ref="formRef">
			<view class="column" v-if="activityData.approveStatus === 'NEW'">
			...
			<view class="column" v-else>
				...
				<up-form-item label="姓名" labelWidth="80" prop="name" :border-bottom="true" :required="true">
					<up-input type="string" v-model="formData.name" :disabled="true" disabledColor="#fff" maxlength="5"
						inputAlign="right" border="none" placeholder="请填写" :clearable="false" />
				</up-form-item>
				<up-form-item label="联系电话" labelWidth="80" prop="phone" :border-bottom="true" :required="true">
					<up-input type="number" v-model="formData.phone" :disabled="true" disabledColor="#fff"
						maxlength="11" inputAlign="right" border="none" placeholder="请填写" :clearable="false" />
				</up-form-item>
				<up-form-item label="学校" labelWidth="80" prop="school" :border-bottom="true" :required="true"
					@click="showPicker('school')">
					<up-input v-model="formData.school" :disabled="true" disabledColor="#ffffff" inputAlign="right"
						border="none" placeholder="请选择"></up-input>
					<template #right>
						<up-icon size="16" name="arrow-right"></up-icon>
					</template>
				...
				<up-form-item label="赛道" labelWidth="80" prop="way" :border-bottom="true" :required="true"
					@click="showPicker('way')">
					<up-input v-model="formData.way" :disabled="true" disabledColor="#ffffff" inputAlign="right"
						border="none" placeholder="请选择"></up-input>
					<template #right>
						<up-icon size="16" name="arrow-right"></up-icon>
					</template>
				...
				<up-form-item label="是否原创" labelWidth="80" prop="isOriginal" :border-bottom="false" :required="true"
					@click="showPicker('isOriginal')">
					<up-input v-model="formData.isOriginal" disabled disabledColor="#ffffff" inputAlign="right"
						border="none" placeholder="请选择"></up-input>
					<template #right>
		...
		<view class="fixed">
			<up-button @click="submit" type="primary" style="color:#ffffff;background-color:#347DFF;border-color:#347DFF;">立即报名</up-button>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async formUrl(form) {
				await post('postSignUpUpdate', form)
				uni.showToast({
					title: "修改成功",
					icon: 'success'
				})
