	...
	<view class="u-upload" :style="[addStyle(customStyle)]">
		<view class="u-upload__wrap">
				...
				<view class="u-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
					...
					<view class="u-upload__status" v-if="item.status === 'uploading' || item.status === 'failed'">
						<view class="u-upload__status__icon">
							...
							<u-loading-icon size="22" mode="circle" v-else />
						</view>
			...
			<template v-if="isInCount">
				<view @tap="chooseFile">
					<slot>
						<view class="u-upload__button" :hover-class="!disabled ? 'u-upload__button--hover' : ''" hover-stay-time="150" @tap.stop="chooseFile" :class="[disabled && 'u-upload__button--disabled']" :style="[{
								width: addUnit(width),
								height: addUnit(height)
							}]">
		...
		<u-preview-video ref="previewVideo"></u-preview-video>
	</view>
</template>

<script>
	import { func, image, video, array, promise } from '@/uni_modules/uview-plus/libs/function/test.js';
	...
	import { addStyle, addUnit, toast, deepClone } from '../../libs/function/index';
...
<style lang="scss" scoped>
	@import '@/uni_modules/uview-plus/libs/css/components.scss';
	@import '@/uni_modules/uview-plus/libs/css/color.scss';
